...
@app.get("/api/admin/coach/ev-hotspots", response_model=EvLeakHotspotsResponse)
def admin_ev_hotspots(window_days: int = Query(default=30, alias="windowDays")) -> EvLeakHotspotsResponse:
    supabase = get_supabase_client()
    parsed_window = 7 if window_days == 7 else 90 if window_days == 90 else 30
    return build_ev_leak_hotspots(supabase, parsed_window)
