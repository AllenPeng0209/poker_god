...
from .services import (
    build_coach_campaign_recommendations,
...
@app.get("/api/admin/coach/campaign-recommendations", response_model=CoachCampaignRecommendationsResponse)
def admin_coach_campaign_recommendations(
    window_days: int = Query(default=30, alias="windowDays"),
) -> CoachCampaignRecommendationsResponse:
    supabase = get_supabase_client()
    parsed_window = 7 if window_days == 7 else 90 if window_days == 90 else 30
    return build_coach_campaign_recommendations(supabase, parsed_window)
