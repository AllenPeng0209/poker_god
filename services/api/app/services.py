...
def get_coach_memory_funnel(min_messages: int = 2) -> CoachAdminMemoryFunnelResponse:
    rid = request_id()
    threshold = max(1, min_messages)
    now = datetime.now(UTC)
    theme_mentions: dict[str, int] = defaultdict(int)
    theme_conversations: dict[str, int] = defaultdict(int)
    total_messages = 0
    active_conversations_24h = 0
    homework_ready_conversations = 0

    for snapshot in _COACH_MEMORY_STORE.values():
        message_count = _safe_int(snapshot.get("message_count"), 0)
        total_messages += message_count
        updated_raw = str(snapshot.get("updated_at") or "")
        try:
            updated_at = datetime.fromisoformat(updated_raw)
            if updated_at.tzinfo is None:
                updated_at = updated_at.replace(tzinfo=UTC)
            if now - updated_at <= timedelta(hours=24):
                active_conversations_24h += 1
        except ValueError:
            pass

        theme_counts = {k: _safe_int(v, 0) for k, v in dict(snapshot.get("theme_counts") or {}).items()}
        if message_count >= threshold and len(theme_counts) > 0:
            homework_ready_conversations += 1

        for key, mentions in theme_counts.items():
            if mentions <= 0:
                continue
            theme_mentions[key] += mentions
            theme_conversations[key] += 1

    conversations = len(_COACH_MEMORY_STORE)
    coverage = round((homework_ready_conversations / conversations) * 100, 1) if conversations > 0 else 0.0
    top_themes = [
        CoachMemoryThemeAggregate(
            key=key,
            mentions=mentions,
            conversationCount=_safe_int(theme_conversations.get(key), 0),
        )
        for key, mentions in sorted(theme_mentions.items(), key=lambda item: item[1], reverse=True)[:5]
    ]

    return CoachAdminMemoryFunnelResponse(
        requestId=rid,
        conversations=conversations,
        totalMessages=total_messages,
        activeConversations24h=active_conversations_24h,
        homeworkReadyConversations=homework_ready_conversations,
        coverageMinMessages=threshold,
        homeworkCoveragePct=coverage,
