...
const coachEndpointTelemetry: CoachEndpointTelemetry[] = [];
...
export function recordCoachEndpointTelemetry(endpoint: string, latencyMs: number, ok: boolean) {
  coachEndpointTelemetry.push({ endpoint, latencyMs: Math.max(0, latencyMs), ok, capturedAt: nowIso() });
  if (coachEndpointTelemetry.length > 5000) {
    coachEndpointTelemetry.splice(0, coachEndpointTelemetry.length - 5000);
  }
}
...
export function getCoachReliabilityAdminSummary(requestId: string): CoachReliabilityAdminSummaryResponse {
  const byEndpoint = new Map<string, { latencies: number[]; errors: number }>();
  for (const item of coachEndpointTelemetry) {
    const current = byEndpoint.get(item.endpoint) ?? { latencies: [], errors: 0 };
    current.latencies.push(item.latencyMs);
    if (!item.ok) current.errors += 1;
    byEndpoint.set(item.endpoint, current);
  }

  const endpoints = Array.from(byEndpoint.entries()).map(([endpoint, value]) => {
    const calls = value.latencies.length;
    const p50LatencyMs = percentile(value.latencies, 0.5);
    const p95LatencyMs = percentile(value.latencies, 0.95);
    const errorRatePct = calls === 0 ? 0 : Number(((value.errors / calls) * 100).toFixed(2));
    return {
      endpoint,
      calls,
      errors: value.errors,
      ...
      p50LatencyMs,
      p95LatencyMs,
      healthy: errorRatePct <= 1 && p95LatencyMs <= 800,
    };
  }).sort((a, b) => b.p95LatencyMs - a.p95LatencyMs);

  const totalCalls = endpoints.reduce((acc, endpoint) => acc + endpoint.calls, 0);
  const totalErrors = endpoints.reduce((acc, endpoint) => acc + endpoint.errors, 0);
  ...
  return {
    ...
    windowLabel: 'runtime',
    totalCalls,
    errorBudgetRemainingPct: Number((Math.max(0, 0.01 - errorRate) / 0.01 * 100).toFixed(1)),
    slowEndpointCount: endpoints.filter((endpoint) => endpoint.p95LatencyMs > 800).length,
    endpoints,
  };
}
